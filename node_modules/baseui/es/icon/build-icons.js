...
async function generateNewIcons() {
  ...
  svgs.forEach(async svgFilename => {
    ...
    let result = iconTemplate.replace(new RegExp('%%ICON_NAME%%', 'g'), componentName).replace(new RegExp('%%ICON_OBJ_PROPS%%', 'g'), props.map(p => `${p.key}: '${p.value}',`).join('')).replace(new RegExp('%%ICON_JSX_PROPS%%', 'g'), props.map(p => `${p.key}="${p.value}"`).join(' ')).replace('%%ICON_PATH%%', reactify(svgFileContents));
    fs.writeFileSync(path.resolve(__dirname, `./${svgFile}.js`), prettier.format(result, {
      parser: 'flow',
      ...prettierOptions
    }));
  });
