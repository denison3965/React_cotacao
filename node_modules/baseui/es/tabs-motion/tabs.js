...
export function Tabs({
  ...
  const uid = useUID(); // Unpack overrides
  ...
  const updateHighlight = React.useCallback(debounce(() => {
    if (activeTabRef.current) {
      setHighlightLayout(getHighlightLayoutParams(activeTabRef.current, orientation));
    }
...
function InternalTab({
  ...
  const {
    ...
    title,
    ...restProps
  } = props; // A way to share our internal activeTabRef via the "tabRef" prop.
  ...
  const [ArtworkContainer, ArtworkContainerProps] = getOverrides(ArtworkContainerOverrides, StyledArtworkContainer); // Keyboard focus styling
  ...
  const handleKeyDown = React.useCallback(event => {
    ...
    const availableTabs = [...event.target.parentNode.childNodes].filter(node => !node.disabled && node.getAttribute('role') === 'tab'); // Exit early if there are no other tabs available
...
function InternalTabPanel({
  ...
  const [TabPanel, TabPanelProps] = getOverrides(TabPanelOverrides, StyledTabPanel); // Keyboard focus styling
