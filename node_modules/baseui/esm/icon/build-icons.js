      ...
      switch (_context2.prev = _context2.next) {
        ...
        case 7:
          ...
          svgs.forEach(function _callee(svgFilename) {
            var svgFile, componentName, svgFileContents, title, viewboxRegex, viewBox, props, result;
              ...
              while (1) {
                switch (_context.prev = _context.next) {
                  case 0:
                    ...
                    result = iconTemplate.replace(new RegExp('%%ICON_NAME%%', 'g'), componentName).replace(new RegExp('%%ICON_OBJ_PROPS%%', 'g'), props.map(function (p) {
                      return "".concat(p.key, ": '").concat(p.value, "',");
                    }).join('')).replace(new RegExp('%%ICON_JSX_PROPS%%', 'g'), props.map(function (p) {
                      return "".concat(p.key, "=\"").concat(p.value, "\"");
                    }).join(' ')).replace('%%ICON_PATH%%', reactify(svgFileContents));
